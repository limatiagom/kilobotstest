...
class KiloBlobs:
        ...
        def __init__(blob,ID,x,y,tamanho):
            blob.ID = ID
            blob.x = x
            blob.y =  y
            blob.tamanho=tamanho
            
            #aqui é um método para printar as informações que serão colocadas no objeto
        def printarInfos(blob):
            print("BLOB ID=%d x = %.1f y= %.1f tamanho=%d" % (blob.ID,blob.x,blob.y,blob.tamanho))
